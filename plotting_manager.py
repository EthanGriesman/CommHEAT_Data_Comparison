...
class PlottingManager:
    ...
    def plot_comparison(
        ...
        show_individual_archetypes: bool = True,  # NEW PARAMETER
    ) -> None:
        """Unified plotting for all comparison types (intersection, heat events, period intersection)
        
        Args:
            combined_data: DataFrame with temperature data
            title: Plot title
            output_path: Path to save the plot
            archetypes: Comma-separated archetype names
            mse_avg: MSE for average temperature (optional)
            mse_max: MSE for max temperature (optional)
            show_individual_archetypes: Whether to show individual archetype lines (default: True)
        """
        ...
        try:
            ...
            # Individual archetypes (if present AND if show_individual_archetypes is True)
            if archetypes and show_individual_archetypes:
    ...
    def plot_heat_event(
        ...
        # Single archetype - show_individual_archetypes doesn't apply
    ...
    def plot_heat_event_averaged(
        ...
        # Default behavior: show individual archetypes for heat events
        self.plot_comparison(combined_data, title, output_path, archetypes,
                           mse_avg, mse_max, show_individual_archetypes=True)

    def plot_period_intersection(
        self, combined_data: pd.DataFrame, address: str, archetypes: str,
        period_start: pd.Timestamp, period_end: pd.Timestamp, output_path: Path
    ) -> None:
        """Plot full period intersection WITHOUT individual archetype lines"""
        title = f"{address}\nPeriod of Intersection: " \
                f"{period_start.strftime('%Y-%m-%d')} to {period_end.strftime('%Y-%m-%d')}\n" \
                f"Averaged Archetypes: {archetypes}"
        # HIDE individual archetypes for period intersection plots
        self.plot_comparison(combined_data, title, output_path, archetypes,
                           show_individual_archetypes=False)
