...
def parse_energyplus_datetime(value: any, target_year: int) -> pd.Timestamp:
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            if t.startswith("24:"):
                t = "00" + t[2:]
                dt = pd.Timestamp(f"{target_year}-{md} {t}")
                return dt + pd.Timedelta(days=1)
            
            # Standard case
            return pd.Timestamp(f"{target_year}-{md} {t}")
    except:
        pass
...
def run_analysis(mapping_df: pd.DataFrame, analysis_type: str, desc: str) -> List[Dict]:
    ...
    # Use ProcessPoolExecutor for CPU-bound operations (MSE calculations)
    if analysis_type in ['mse', 'period_intersection_means']:
        # CPU-bound operations - use processes
        max_workers = max(1, multiprocessing.cpu_count() - 1)
        executor_class = ProcessPoolExecutor
    else:
        # I/O-bound operations (plotting) - use threads
